...
1. **Shell history.** If you run `npx -y @apier/mcp` with the key inlined on the command line, your shell will record it. Prefer the `APIER_API_KEY` env variable.
...
Please include reproduction steps and the version of `@apier/mcp` you tested against. We aim to acknowledge within 48 hours and to publish a fix within 7 days for high-severity issues.
