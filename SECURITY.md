...
1. **API key leakage into spawned child processes.** `APIER_API_KEY` is read by the parent and removed from the env object passed to `mcp-remote`, along with every other variable whose name contains `TOKEN`/`SECRET`/`BEARER`/`KEY`/`PASSWORD`/`CREDENTIAL`/`APIER`. The bearer value is handed over out-of-band in a single dedicated variable, `APIER_MCP_AUTH_HEADER`, which `mcp-remote` expands into its `Authorization` request header. `APIER_API_KEY` under its own name never reaches the child.
...
3. **Local process inspection.** The key is no longer exposed on the child's command line — as of 1.2.0 the `--header` argv element carries only a placeholder, and the bearer value travels in the child's environment instead (see "What we defend against" #2). This matters because the two are not equally protected: on Linux `/proc/<pid>/cmdline` is world-readable, so *any* local user could previously read the key out of a running proxy, whereas `/proc/<pid>/environ` is readable only by the process owner and root.
   ...
   What remains: an attacker who is already running as your user, or as root, can still read the value out of the process environment (`/proc/<pid>/environ`, a debugger, a core dump) — as they could read it from your MCP client's config file or your shell environment anyway. This package runs in user space and cannot defend against an attacker who already has your privileges. Rotate the key at https://www.apier.no/dashboard/keys if you suspect local compromise.
